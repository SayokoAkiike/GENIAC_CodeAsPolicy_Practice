...
**Status:** Step 1 (cascade) now has a real, verified result: 33.33% → 100.00% on `train_mini.yaml`, with Groq's addition directly responsible for half of that (rescuing 9/18 tasks after Gemini's quota ran out). Steps 3, 4, and 5 are still only validated with fake/mock clients (🧪 rows above) — those are tuning-based techniques, so their real numbers should come from `benchmarks/hard_benchmark_v1_test.yaml` (held out, touched once) per `docs/rigorous-verification-plan.md`, not from ad-hoc runs. Next concrete action: re-run `harvest-vocabulary`, `hill-climb-prompt`, and `bandit-cascade` for real against `train`/`train_mini`, now that the 3-tier fallback means Gemini's quota no longer blocks a full session.
